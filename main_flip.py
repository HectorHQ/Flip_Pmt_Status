...
with col1:
    ...
    pmt_status = st.radio('Select Payment Status',options=['Paid','Remitted','Partial Paid','Self Collected','90 Days Fees Collected','Processing','Write Off','Third Party Collections','Unpaid'],index=1)
    if pmt_status == 'Paid':
...
with col2:
    ...
    elif pmt_status == 'Unpaid':
        try:
            st.title(f'You have selected {pmt_status} Status.')
            st.warning('Be sure all the information is correct before submitting.')
            submit_to_Unpaid = st.button('Submit to Unpaid')
            if submit_to_Unpaid:
                headers = connect_website(bearer_token)
                flip_to_Unpaid(headers,df['Invoice'])
        except NameError:
            st.write('Error, reach out to admin')        
